...
def sync_thickness_from_item_attribute(doc, method):
    ...
    for row in doc.item_attribute_values:
        ...
        thickness_name = frappe.db.exists(
            "Thickness",
            {
                "reference": doc.name,
...
def sync_thickness_delete(doc, method):
    ...
    for value in removed_values:
        ...
        if in_use:
            frappe.throw(
                f"Cannot remove thickness {value}. It is used in Item Variants."
            )

        thickness_doc = frappe.db.get_value(
            "Thickness",
            {
                "reference": doc.name,
                "value": value
...
def sync_thickness_delete_row(doc):
    ...
    current_values = {
        row.attribute_value for row in doc.item_attribute_values
    }
    ...
    for t in thickness_records:
        if t.value not in current_values:
            ...
            in_use = frappe.db.exists(
                ...
                {
                    "attribute": doc.name,
                    "attribute_value": t.value
                }
            )

            if in_use:
                frappe.throw(
                    f"Cannot remove thickness {t.value}. It is used in Item Variants."
                )
